//define DEBUG for printing some debug information on the serial
#define DEBUG true
#if (DEBUG)
  #define DEBUG_WRITE(debug_msg) Serial.print(debug_msg)
  #define DEBUG_WRITELN(debug_msg) Serial.println(debug_msg)
#else
  #define DEBUG_WRITE(debug_msg)
  #define DEBUG_WRITELN(debug_msg)
#endif
...
void mqttconnect() {
  ...
  while (!client.connected()) {
    DEBUG_WRITE("MQTT connecting ...");
    /* client ID */
    String clientId = "ESP32Client";
    /* connect now */
    if (client.connect(clientId.c_str())) {
      DEBUG_WRITELN("connected");
      /* subscribe topic */
      client.subscribe(LED_TOPIC);
    } else {
      DEBUG_WRITE("failed, status code =");
      DEBUG_WRITE(client.state());
      DEBUG_WRITELN("try again in 5 seconds");
...
void setup() {
  ...
  DEBUG_WRITELN("");
  DEBUG_WRITELN("WiFi connected");
  DEBUG_WRITELN("IP address: ");
  DEBUG_WRITELN(WiFi.localIP());
  /*setup MDNS for ESP32 */
  if (!MDNS.begin("esp32")) {
      DEBUG_WRITELN("Error setting up MDNS responder!");
      while(1) {
          delay(1000);
      }
  }
  /* get the IP address of server by MDNS name */
  DEBUG_WRITELN("mDNS responder started");
  IPAddress serverIp = MDNS.queryHost(serverHostname);
  DEBUG_WRITE("IP address of server: ");
  DEBUG_WRITELN(serverIp.toString());
